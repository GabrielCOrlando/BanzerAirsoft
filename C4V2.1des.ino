...
#define rele1 2 //pino do rele sirene
...
void contador(int tempoinicial) { //função para rotina de acionamento da bomba - começa a contagem de tempo da bomba ativada.
  ...
  while (count > 0) { //enquanto o tempo de jogo for maior que zero...
    ...
    if (temporestante == 900) { //15min  //rotinas para mudança da velocidade do bip
      intervalo = 800;
    }
    if (temporestante == 600) { //10min
      intervalo = 700;
    }
    if (temporestante == 300) { //5min
      intervalo = 600;
    }
    ...
    if (temporestante == 30) {  //30seg
      intervalo = 150;
    }
    if (temporestante == 10) {  //10seg
    intervalo = 75;
    }
    if (temporestante == 4) {  //2 seg
      intervalo = 1;
    }
